...
execute \
  as @e[x=0,y=0,z=0,type=minecraft:text_display,tag=egg.coord.egg_math.x,limit=1] \
    run data modify entity @s Rotation[0] set from storage egg:math/cos <<degree~double
## Get sin and cos.
execute \
...
## Set output data.
data modify storage egg:math/cos >>cos~double set from storage egg:data/3d-double_from_position >>3d~double[2]
