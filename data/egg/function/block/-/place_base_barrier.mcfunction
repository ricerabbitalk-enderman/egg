...
## New interaction.
data modify storage egg:block/-/interaction/new << set value {cooltime:5}
...
execute as @e[distance=..0.1,type=minecraft:item_display,tag=egg.block.view] if function egg:uuid/match run tag @s add egg.block.base.barrier
